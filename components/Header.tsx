    ...
    <header className={`fixed top-0 w-full z-40 transition-all duration-500 ${isScrolled ? 'bg-white/95 backdrop-blur-md shadow-lg py-2' : 'bg-transparent py-4 md:py-6'}`}>
      <div className="container mx-auto px-4 md:px-6 flex justify-between items-center">
        <div className="flex items-center gap-3 md:gap-6 group cursor-pointer flex-1 md:flex-none">
          <div className={`transition-all duration-500 flex items-center justify-center overflow-hidden flex-shrink-0 ${isScrolled ? 'w-12 h-12 md:w-16 md:h-16' : 'w-16 h-16 md:w-24 md:h-24 lg:w-28 lg:h-28'}`}>
            {logo ? (
              <img 
                src={logo} 
                alt="Logo C.D.S" 
              ...
              <div className="w-full h-full bg-[#800000] rounded-xl flex items-center justify-center">
                <span className={`text-white font-bold transition-all ${isScrolled ? 'text-sm' : 'text-xl'}`}>CDS</span>
              </div>
            )}
          </div>
          <div className="flex flex-col min-w-0">
            <span className={`font-serif font-bold tracking-tight transition-all duration-500 text-[#800000] truncate ${isScrolled ? 'text-lg md:text-xl' : 'text-xl md:text-3xl'}`}>
              Clínica Dente Saudável
            </span>
            {!isScrolled && (
              <span className="text-[#B19CD9] text-[10px] md:text-xs font-bold tracking-[0.2em] uppercase mt-0 md:mt-1 animate-pulse whitespace-nowrap">
        ...
        <div className="xl:hidden ml-2">
          <a
            href="#contato"
            className={`transition-all duration-500 bg-[#800000] text-white rounded-full font-bold shadow-md flex items-center justify-center ${isScrolled ? 'w-10 h-10' : 'w-12 h-12'}`}
          >
